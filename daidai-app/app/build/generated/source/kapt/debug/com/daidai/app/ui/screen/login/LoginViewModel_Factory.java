// Generated by Dagger (https://dagger.dev).
package com.daidai.app.ui.screen.login;

import com.daidai.app.data.local.ServerConfig;
import com.daidai.app.data.remote.TokenManager;
import com.daidai.app.data.repository.AuthRepository;
import dagger.internal.DaggerGenerated;
import dagger.internal.Factory;
import dagger.internal.QualifierMetadata;
import dagger.internal.ScopeMetadata;
import javax.inject.Provider;

@ScopeMetadata
@QualifierMetadata
@DaggerGenerated
@SuppressWarnings({
    "unchecked",
    "rawtypes",
    "KotlinInternal",
    "KotlinInternalInJava"
})
public final class LoginViewModel_Factory implements Factory<LoginViewModel> {
  private final Provider<AuthRepository> authRepositoryProvider;

  private final Provider<TokenManager> tokenManagerProvider;

  private final Provider<ServerConfig> serverConfigProvider;

  public LoginViewModel_Factory(Provider<AuthRepository> authRepositoryProvider,
      Provider<TokenManager> tokenManagerProvider, Provider<ServerConfig> serverConfigProvider) {
    this.authRepositoryProvider = authRepositoryProvider;
    this.tokenManagerProvider = tokenManagerProvider;
    this.serverConfigProvider = serverConfigProvider;
  }

  @Override
  public LoginViewModel get() {
    return newInstance(authRepositoryProvider.get(), tokenManagerProvider.get(), serverConfigProvider.get());
  }

  public static LoginViewModel_Factory create(Provider<AuthRepository> authRepositoryProvider,
      Provider<TokenManager> tokenManagerProvider, Provider<ServerConfig> serverConfigProvider) {
    return new LoginViewModel_Factory(authRepositoryProvider, tokenManagerProvider, serverConfigProvider);
  }

  public static LoginViewModel newInstance(AuthRepository authRepository, TokenManager tokenManager,
      ServerConfig serverConfig) {
    return new LoginViewModel(authRepository, tokenManager, serverConfig);
  }
}
